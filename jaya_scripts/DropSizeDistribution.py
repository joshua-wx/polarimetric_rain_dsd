...
from .partition import cs_partition_islam_2012
...
class DropSizeDistribution(object):
    ...
    def __init__(self, reader, time_start = None, location=None):
        ...
        self.nd=reader.nd    
        self.rain_type=None
        self.rain_group=None
        self.Nd=None
        self.sigma=None
        try:
            self.filtered_raw_matrix=reader.filtered_raw_matrix
        except:
            self.filtered_raw_matrix=None


    def apply_filter(self):
        self.fields['Nd'] = common.var_to_dict(
            'Nd', np.ma.masked_equal(self.ndf,-9.99), 'm^-3 mm^-1',
            'Liquid water particle concentration')
        self.fields['Nd']['data'].set_fill_value(1)
        self.Nd=self.fields['Nd']


    def apply_raw_data(self):
        nd=np.zeros(shape=(len(self.time['data']),32), dtype=float)
        for t in range(0,len(self.time['data'])):
            for i in range(2,21):
                if self.nd[t][i]==0:
                    nd[t][i]=0
                else:
        ...
        self.fields['Nd'] = common.var_to_dict(
            'Nd', np.ma.masked_equal(nd, np.power(10, -9.999)), 'm^-3 mm^-1',
            'Liquid water particle concentration')
        self.fields['Nd']['data'].set_fill_value(1)
        self.Nd=self.fields['Nd']
    ...
    def calculate_radar_parameters(self, dsr_func = DSR.bc, scatter_time_range = None ):
        ...
        for t in range(self.scatter_start_time, self.scatter_end_time):
            ...
            self.fields['AiH']['data'][t] = radar.Ai(self.scatterer,h_pol=True)
            self.fields['AiV']['data'][t] = radar.Ai(self.scatterer,h_pol=False)
            self.fields['Adr']['data'][t] = radar.Ai(self.scatterer) -radar.Ai(self.scatterer, h_pol=True)

    def _setup_empty_fields(self ):
        ''' Preallocate arrays of zeros for the radar moments
        '''
        params_list = ['Zh', 'Zdr', 'Kdp', 'AiH', 'Adr','AiV']
    ...
    def calculate_dsd_parameterization(self, method='bringi'):
        ...
        c=np.zeros(shape=self.numt)

        for t in range(0, self.numt):
            if np.sum(self.Nd['data'][t]) == 0:
                continue
            self.fields['Nt']['data'][t] = np.sum(np.dot(self.spread['data'], self.Nd['data'][t]))
            self.fields['W']['data'][t] = vol_constant * np.dot(np.multiply(self.Nd['data'][t], self.spread['data']),np.array(self.diameter['data']) ** 3)
            self.fields['D0']['data'][t] = self._calculate_D0(self.Nd['data'][t])
            self.fields['Nw']['data'][t] =  256.0 /(np.pi * rho_w) * np.divide(self.fields['W']['data'][t],self.fields['Dm']['data'][t] ** 4)
            self.fields['Dmax']['data'][t] = self.__get_last_nonzero(self.Nd['data'][t])            
            #M6[t]=np.sum(np.log10(self.Nd['data'][t])*self.diameter['data']**6)
            c[t]=np.sum((self.diameter['data']-self.fields['Dm']['data'][t])**2*self.spread['data']*self.Nd['data'][t]*self.diameter['data']**3)
        
        self.c=c
        self.fields['mu']['data'][:] = list(map(self._estimate_mu,list(range(0,self.numt))))
        A=(self._calc_mth_moment(3)**2/(self._calc_mth_moment(2)*self._calc_mth_moment(4)))
        self.mu=(3-4*A)/(A-1)  #mu calculated from moments
        self.Nw=1/(gamma(4)/4**4)*(self._calc_mth_moment(3)**5/self._calc_mth_moment(4)**4)
        self.Zh=10*np.log10(self._calc_mth_moment(6))
        self.sigma=(c/self._calc_mth_moment(3))**0.5
    ...
    def calculate_RR(self):
        ...
        for t in range(0, self.numt):
            ...
            self.fields['rain_rate']['data'][t] = 0.6 * np.pi * 1e-03 * np.sum(self._mmultiply(
                velocity,nd, self.spread['data'], np.array(self.diameter['data']) ** 3))
    ...
    def calculate_R_Ai_relationship(self):
        '''
        calculate_R_Ai_relationship calculates a power fit for the rainfall-Attenuation
        relationship based upon the calculated radar parameters(which should
        have already been run). It returns the scale and exponential
        parameter a and b in the first tuple, and the second returned argument
        gives the covariance matrix of the fit.
        ...
        if 'rain_rate' in list(self.fields.keys()):
            filt = np.logical_and(
                self.fields['Ai']['data'] > 0, self.fields['rain_rate']['data'] > 0)
            popt, pcov = expfit(self.fields['Ai']['data'][filt],
                                self.fields['rain_rate']['data'][filt])
    ...
    def _estimate_mu(self, idx):
        ...
        if np.sum(self.Nd['data'][idx]) == 0 :
            return np.nan
        res = scipy.optimize.minimize_scalar(self._mu_cost, bounds = (-5,20), args = (idx,), method='bounded')
    ...
    def classification(self):
        self.rain_type = cs_partition_islam_2012(self.rain_rate['data'])
        
       
    def rain_spectrum(spectrum=np.array([0,0.1,1,5,10,100])):
        """
        This splits the whole data into spectrum based on rainfall rate defined in the spectrum
        ...
        Returns : 
            group of rain spectrum
           
        """
        df=pd.Series(self.fields['rain_rate']['data'])
        ind=np.digitize(df,spectrum)
        self.rain_group=ind
    
    
    def save_result(self):
        result=pd.DataFrame()
        result['Rain rate']=self.fields['rain_rate']['data']
        result['Zh']=self.fields['Zh']['data']
        result['Zdr']=self.fields['Zdr']['data']
        result['Kdp']=self.fields['Kdp']['data']
        result['AiH']=self.fields['AiH']['data']
        result['AiV']=self.fields['AiV']['data']
        result['Freq']=self.scattering_freq
        #result['D0']=self.fields['D0']['data']
        #result['Dm']=self.fields['Dm']['data']
        #result['Nw']=self.fields['Nw']['data']
        #result['mu']=self.fields['mu']['data']
        result.index=self.time['data']
        #nd=pd.DataFrame(self.fields['Nd']['data'],index=result.index)
        #res=pd.concat([result,nd],axis=1)
        return result
        
    def save_raw_result(self):
        result=pd.DataFrame()
        result['Rain rate']=self.rain_rate['data']
        result['Zh']=self.Z['data']
        result.index=self.time['data']
        nd=pd.DataFrame(self.fields['Nd']['data'],index=result.index)
        ndf=pd.DataFrame(self.ndf,index=result.index)
        res=pd.concat([result,nd,ndf],axis=1)
        return res
